...
UPDATE_BASE=${HOME}/Documents/contribs/kde-frameworks
UPDATE_REPO="${UPDATE_BASE}/$(echo $fork | awk -F'/' '{ print $NF }')"
LOG_DIR=${HOME}/Documents/contribs/kde-frameworks/logs
DATE=$(date +%Y-%m-%d)
cwd=$(pwd)
LIST=$cwd/kde-frameworks.lst
...
git push
git checkout -b $DATE-kde-frameworks-update
